...
const RecoveryConfirm: React.FC<RecoveryConfirmProps> = ({ children }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const codeErr = validate(code);
    const passErr = validate(password, "password");

    if (codeErr || passErr) {
      dispatch({
        type: SET.SET_MESSAGE_DATA,
        payload: "Please fix the errors before submitting",
      });
    }
    ...
    const convData = JSON.stringify({
      code,
      password,
      getInfo: window.navigator.userAgent,
    });
        ...
        <form method="POST" onSubmit={handleSubmit}>
          <div className="field__wrapper">
            <Field
              ...
              value={state.code}
              style={{
                borderColor: state.messageCode ? "rgb(217, 43, 73)" : "",
              }}
            ></Field>
            <Field
