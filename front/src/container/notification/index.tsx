  ...
  const handleCheck = (id: number) => async () => {
    try {
      const res = await fetch(
        `http://localhost:4000/notifications/update?id=${id}`
      );
      ...
      <Section>
        ...
        <div className="notifications__list">
          {data.length !== 0 ? (
            data
              ?.slice()
              .sort((a, b) => b.notificationId - a.notifinicationId)
              .map((note) => (
                <React.Fragment key={note.notificationId}>
                  <List
                    onItemClick={handleCheck(note.notificationId)}
                    className={
                      note.action === `notification` ? `notification` : `warn`
                    }
                    style={{
                      ...(note.ifUnread === false
                        ? {}
                        : {
                            border: `8px solid rgba(217, 43, 73, 0.5)`,
                            borderRadius: `50%`,
                          }),
                    }}
                    title={`New ${note.action.toUpperCase()} by ${note.name}`}
                    info=""
                    details={whenNotificationSent(
                      note.date,
                      new Date().getTime()
                    )}
