...
const SettingsStage: React.FC<settingsProps> = ({ children }) => {
  ...
  const handleNewPassSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!passwordOld || !passwordNew || passOldError || passNewError) {
      dispatch({
        type: SET.SET_MESSAGE_DATA,
        payload: "Error, u have to enter correct data",
      });
    }

    const id = JSON.parse(window.localStorage.sessionAuth).user.userId;
    const convertData = JSON.stringify({
      currentData: passwordOld,
      ...
      <Section>
        ...
        >
          ...
          <Button
            type="submit"
            className={`button ${
              isEmailFormFilled ? "button--primary" : "button--secondary"
            }`}
        ...
        >
          ...
          <Button
            type="submit"
            className={`button ${
              isPasswordFormFilled ? "button--primary" : "button--secondary"
            }`}
            style={{ padding: "16px 0", fontSize: "14px", fontWeight: "400" }}
          >
            Save Password
